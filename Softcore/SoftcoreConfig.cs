            ...
            try
            {
                using (UMFConfig cfg = new UMFConfig())
                {
                    ...
                    if (cfgVer != string.Empty && cfgVer != configVersion)
                    {
                        ...
                        Softcore.Log("The config file was outdated and has been deleted. A new config will be generated.");
                    }
                    ...
                    Softcore.Log("Finished UMF Settings.");

                    //Add your settings here

                    Softcore.Log("Finished loading settings.");
                }
            }
            catch (Exception e)
            {
                Softcore.Log("Error loading mod settings: " + e.Message + "(" + e.InnerException?.Message + ")");
            }
        }
    }
}